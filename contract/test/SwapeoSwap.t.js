...
describe("SwapeoSwapTest", function () {
  let swapeo;
  let tokenA;
  let tokenB;
  let tokenC;
  ...
  describe("Events and returns", function () {
    ...
    it("test_swap_emitsSwapEvent_withCorrectValues", async function () {
      ...
      const fee = amountIn * BigInt(3) / BigInt(1000);
    ...
    it("test_swap_doesNotEmitEventOnRevert", async function () {
      const amountIn = ethers.parseEther("1");
      const expectedOut = await swapeo.getAmountOut(amountIn, tokenA.target, tokenB.target);
      const tooLowMin = expectedOut + BigInt(1); // provoque un revert
      
      await tokenA.connect(addr1).approve(swapeo.target, amountIn);
    
      await expect(
        swapeo.connect(addr1).swap(tokenA.target, tokenB.target, amountIn, tooLowMin)
  ...
  describe("UnhappyPath", function () {
    it("test_swap_revertsOnZeroAmount", async function () {
      await expect(swapeo.swap(tokenA.target, tokenB.target, 0, 0)).to.be.revertedWith("Invalid amount");
    });
    ...
    it("test_swap_revertsIfOutputTooLow", async function () {
      ...
      await expect(
        swapeo.connect(addr1).swap(tokenA.target, tokenB.target, amountIn, expected + BigInt(1))
      ).to.be.revertedWith("High slippage");
    });
    ...
    it("test_swap_revertsOnNonexistentPair", async function () {
      const amountIn = ethers.parseEther("1");
      await tokenA.connect(addr1).approve(swapeo.target, amountIn);
      await expect(swapeo.connect(addr1).swap(tokenA.target, tokenC.target, amountIn, 1)).to.be.revertedWith("Use forwardToUniswap");
    });
  });
