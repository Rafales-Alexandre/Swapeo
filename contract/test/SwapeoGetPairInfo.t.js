...
describe("SwapeoGetPairInfoTest", function () {
  let swapeo;
  let tokenA;
  let tokenB;
  let tokenC;
  ...
  describe("HappyPath", function () {
    ...
    it("test_getPairInfo_orderIndependence", async function () {
      const [info1A, info1B] = await swapeo.getPairInfo(tokenA.target, tokenB.target);
      const [info2A, info2B] = await swapeo.getPairInfo(tokenB.target, tokenA.target);

      expect(info1A).to.equal(info2A);
      expect(info1B).to.equal(info2B);
    });
    ...
    it("test_getPairInfo_afterSwap_reflectsUpdatedReservesAndFees", async function () {
      ...
      const [token0, token1, reserveBeforeA, reserveBeforeB, ,] = before;
      ...
      const [, , reserveAfterA, reserveAfterB, , fees] = after;
    
      // Déduire dans quel ordre sont stockés les tokens
      const isToken0Input = token0 === tokenA.target;
    
      if (isToken0Input) {
        expect(reserveAfterA).to.be.gt(reserveBeforeA); // Token A (input) augmente
        expect(reserveAfterB).to.be.lt(reserveBeforeB); // Token B (output) diminue
      } else {
        expect(reserveAfterB).to.be.gt(reserveBeforeB); // Token A (input) augmente
        expect(reserveAfterA).to.be.lt(reserveBeforeA); // Token B (output) diminue
      }
    ...
    it("test_getPairInfo_afterDistributeFees_resetsFeesCollected", async function () {
      ...
      await tokenA.transfer(addrList[1].address, ethers.parseEther("10"));
      await tokenA.connect(addrList[1]).approve(swapeo.target, ethers.parseEther("10"));
    
      const amountIn = ethers.parseEther("1");
      const amountOutMin = await swapeo.getAmountOut(amountIn, tokenA.target, tokenB.target);
      await swapeo.connect(addrList[1]).swap(tokenA.target, tokenB.target, amountIn, amountOutMin - amountOutMin / 100n);
    
      await swapeo.distributeFees(tokenA.target, tokenB.target);
    
      const [, , , , , feesCollected] = await swapeo.getPairInfo(tokenA.target, tokenB.target);
      expect(feesCollected).to.equal(0);
    });
