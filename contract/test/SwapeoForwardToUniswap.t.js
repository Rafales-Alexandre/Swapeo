...
describe("SwapeoForwardToUniswapTest", function () {
  let swapeo;
  let tokenA;
  let tokenB;
  let owner;
  ...
  beforeEach(async function () {
    ...
    swapeo = await SwapeoDEX.deploy(uniswapRouter.target);
  ...
  describe("Happy Path", function () {
    ...
    it("test_forwardToUniswap_emitsExpectedEvent", async function () {
        ...
        ).to.emit(swapeo, "ForwardedToUniswap");
      });
      ...
      it("test_forwardToUniswap_contractDoesNotKeepInputToken", async function () {
        ...
        const tx = await swapeo.connect(addr1).forwardToUniswap(tokenA.target, tokenB.target, amountIn, 0);
        await tx.wait();
  ...
  describe("Unhappy Path", function () {
    ...
    it("test_forwardToUniswap_revertsIfAllowanceTooLow", async function () {
        await tokenA.connect(addr1).approve(swapeo.target, 0);
        await expect(
          swapeo.connect(addr1).forwardToUniswap(tokenA.target, tokenB.target, ethers.parseEther("1"), 0)
